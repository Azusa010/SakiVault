...
export interface MusicStreamResult {
  response: Response
  source: MusicUrlResult['source']
}

// 获取歌曲的远程音频流,
export async function getMusicStream(
  params: MusicUrlParams,
  range?: string,
): Promise<MusicStreamResult> {
  const playable = await getPlayableUrl(params)

  if (!playable.url) {
    throw new Error('没有可代理的音频地址')
  }
  ...
  if (!response.ok && response.status !== 200) {
    throw new Error(`音频源响应异常:${response.status}`)
  }
